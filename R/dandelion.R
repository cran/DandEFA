dandelion <- function(fact_load,bound=0.5)
{
  ...
  {
    colseq <- seq(0,1,by=0.01)
    col1 <- rgb(1-colseq,0.4,colseq)
    col2 <- rgb(colseq,0.4,1-colseq)
    x4 <- seq(tempx,x3,by=(x3-tempx)*(1/100))
    y4 <- seq(tempy,y3,by=(y3-tempy)*(1/100))
    
    # Shifting from Red to Blue 
    if(fact_load1 > 0 && fact_load2 < 0)
    { 
      for( k in 2:100 )
        polygon(c(x4[k],x2,x4[k-1]),c(y4[k],y2,y4[k-1]),col=col1[k],border=col1[k])
    }
    # Shifting from Blue to Red 
    else if(fact_load1 < 0 && fact_load2 > 0) 
    { 
      for( k in 2:100 )
        polygon(c(x4[k],x2,x4[k-1]),c(y4[k],y2,y4[k-1]),col=col2[k],border=col2[k])
    }
    else if(fact_load1 < 0 && fact_load2 < 0) 
      polygon(c(x3,x2,tempx),c(y3,y2,tempy),col=col1[1],border=col1[1])
    
    else if(fact_load1 > 0 && fact_load2 > 0) 
      polygon(c(x3,x2,tempx),c(y3,y2,tempy),col=col2[1],border=col2[1])
  }
  ##################################################################################
  ...
  plot(1,type="n",axes=FALSE,xlab="",ylab="",xlim=c(xmin,xmax),ylim=c(ymin,ymax))
  box()
  ...
  for ( i in 1:factor) 
  {
    ...
    y3 = y2[i] + (limit[i]*datanew)*(sin(-degreev*pi/180))
    
    if(fact_load[1,i] > 0)  col2 = rgb(0,0.4,1) 
    if(fact_load[1,i] < 0)  col2 = rgb(1,0.4,0)
    polygon(c(x3[1],x2[i]),c(y3[1],y2[i]),col=col2,border=col2)    
    lines(c(x3[1],xm[1]),c(y3[1],ym[1]), type="l",col="grey")
  ...
  #####################################################################################
  # Ploting Uniquenesses and communalities 
