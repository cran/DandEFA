dandelion <- function (fact_load, bound = 0.5, mcex = c(1, 1), palet) 
{
    ...
    if (max(limitt) > abs(ymin)) 
        ymin <- -1 * max(limitt)
    par(mar = c(0, 0, 0, 0))
    layout(matrix(c(1, 1, 1, 1, 2, 3, 4, 5), nrow = 4), widths = c(7, 3), heights = c(0.5, 1, 1, 1))
    ...
    plot(1:10, type = "n",xlim = c(-1.5, 1.5), ylim = c(-1.5, 1.5), axes = FALSE)
    legend(0, 0, c("pos. load.", "neg. load."), col = c(palet[1],palet[length(palet)]),
       text.col = "black", bg = "white", xjust=0.5, yjust=0.5, 
       pch=c(15,15),cex=1.5)
    par(mar = c(0, 0, 0.9, 0))
    ...
    colnames(ara) <- c("Var.", "Cum. Ratio")
    textplot(ara, valign = "top", halign = "center", mar = c(0,0, 0, 0))
